...
export const prepararSolicitudNotificacion = (
    ...
    const index = solicitudes.findIndex(
        solicitud => solicitud.id.toString() === notificacion.idSolicitud.toString()
    );
    ...
    if (index !== -1) {
        ...
        const comentarioYaExiste =
            solicitudExistente.comentariosFuncionario?.includes(notificacion.comentarioDetalle) || false;
